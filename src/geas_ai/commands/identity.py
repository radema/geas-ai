...
def add_identity(
    ...
    persona: str = typer.Option(None, "--persona", "-p", help="Persona (required if role is agent)."),
    model: str = typer.Option(None, "--model", "-m", help="Model (required if role is agent)."),
):
    """
    ...
    try:
        ...
        if role not in [r.value for r in IdentityRole]:
            console.print(f"[bold red]Error:[/bold red] Invalid role '{role}'. Must be 'human' or 'agent'.")
        ...
        if key_path.exists():
             console.print(f"[bold red]Error:[/bold red] Key file already exists: {key_path}")
             raise typer.Exit(code=1)
...
def list_identities():
    ...
    for i in store.identities:
        ...
        key_display = i.active_key.split()[1][:16] + "..." if len(i.active_key.split()) > 1 else "Invalid"
        table.add_row(
            i.name,
            i.role.value,
            i.persona or "-",
